...
사용법
------
    python build_backlinks.py  (DATABASE_URL 환경변수 필요)
"""
import json
...
def build() -> None:
    ...
    with conn.cursor() as cur:
        cur.execute(
            "SELECT category, item_id, label, links_json FROM raw_attrs "
            "WHERE links_json IS NOT NULL AND links_json != '[]'"
        )
        for category, item_id, label, links_json in cur:
    ...
    with conn.cursor() as cur:
        cur.execute("SELECT category, item_id, label, rows_json FROM raw_tables")
        for category, item_id, label, rows_json in cur:
